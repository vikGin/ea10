var app = (function () {
    ...
    function initShaderProgram() {
        ...
        gl.bindAttribLocation(prog, 2, "aTextureCoord");
        gl.linkProgram(prog);
        gl.useProgram(prog);
    }
    ...
    function initProceduralTexture(model) {
        var texture = gl.createTexture();
        model.texture = texture;

        // Create two stripes (brown top, beige bottom) with red dots on brown
        var size = 256;
        ...
        for (var i = 0; i < size * size; i++) {
            ...
            if (v < 0.5) {
                // Top half: Green color with wavy horizontal stripes
                var stripeHeight = 0.1;
                // Add wave effect based on horizontal position
                var wave = Math.sin(u * 6 * Math.PI) * 0.03;
                var adjustedV = v + wave;
                var stripeIndex = Math.floor((adjustedV / 0.5) / stripeHeight) % 2;
                ...
                // Bottom half: Light brown color
                r = 250;
                g = 180;
                b = 140;
            }

            // Check for red dots on green section (top half)
            if (v < 0.5) {
                // Define three dots with center and radius
                var dots = [
                    { cx: 0.4, cy: 0.25, radius: 0.07 },
                    { cx: 0.6, cy: 0.35, radius: 0.13 },
    ...
    function onloadTextureImage(texture) {
        ...
        // Texture wrapping for seamless repetition
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    ...
    function initModels() {
        ...
        // First torus with loaded texture
        createModel("torus", fs, [1, 1, 1, 1], [-0.6, 0.6, 0, 0], [1.9, 0.2, 0.5,
            0], [1, 1, 1, 1], mGrey, "textures/donut.png");
        // Second torus with procedural texture
        createModel("torus", fs, [1, 1, 1, 1], [1, 0.6, 0, 0], [1.9, 0.2, 0.5,
            0], [1, 1, 1, 1], mGrey, null, true);
     ...
     * @parameter textureFilename: filename for texture image or null.
     * @parameter useProcedural: boolean to use procedural texture instead of image.
     */
    function createModel(geometryname, fillstyle, color, translate, rotate,
        scale, material, textureFilename, useProcedural) {
        var model = {};
    ...
    function render() {
        ...
        for (var i = 0; i < models.length; i++) {
            ...
            gl.uniform1i(prog.hasTextureUniform, models[i].texture && models[i].texture.loaded ? 1 : 0);
    ...
    function updateTransformations(model) {
        ...
        // Apply scene rotation (from arrow keys).
        mat4.rotateX(mMatrix, mMatrix, camera.rotateX);
        mat4.rotateY(mMatrix, mMatrix, camera.rotateY);
        // Rotate.
